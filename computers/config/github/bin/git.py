...
def work_with_file_from_script_directory(file):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    abs_path = os.path.join(script_dir, file)
    return abs_path
...
def local_or_global_for_git_commands():
    try:
        work_with_file_from_script_directory("data/git_commands.js")
...
def check_if_file_exists(filename):
    ignore_file = work_with_file_from_script_directory(".ignore.txt")
    ...
    class Synch:
        ...
        def __init__(self, parent):
            ...
            self.commands = {
              ...
              "remove_remote": "git remote remove origin"
            }
        ...
        def remove_remote(self):
            ...
            command = self.commands["remove_remote"].split(" ") 
            sp.run(command)
            # print(command)
        def activate(self):
            ...
            return True
        def to_main(self):
            """ Make current branch the main branch """
            files = os.listdir()
            files.remove(".git")
            directory = self.parent.repository_name
            clone = f"git clone https://github.com/{self.parent.username}/{directory}.git"
            sp.run(clone.split(" "))
            for file in files:
                if os.path.isdir(file):
                    sp.run(['mv', '-r', file, directory])
                else:
                    sp.run(['mv', file, directory])
            os.chdir(directory)
            sp.run(['git', 'init'])
            sp.run(['set.py', '-b', 'main', '-u', self.parent.username, '-r', directory, '-t', 'na'])
            self.remove_remote()
            self.remote()
            sp.run(['gfs.py'])
            return True
...
def main():
    try:
        ...
        print("\n\n### no keys/github found, please put token using 'gset' ###\n\n")
    try:
        with open(".git_config.json", 'r') as file:
            changes = json.load(file)
    except FileNotFoundError:
        print("\n\n### No configurations were found; create one using 'gset' ###\n\n")
